...
Should you use `interface` or `type`? It depends. Should you avoid `any` at all costs? It depends. Should you keep strict mode on? Yes. We have a few opinions.
...
The examples target **TypeScript 7**, checked with **7.0.2**. Strict mode is on by default in TypeScript 7, and we keep it on. The checks also enable `exactOptionalPropertyTypes`, except when comparing its behavior with the flag off.

We leave `noUncheckedIndexedAccess` off. The configuration generated by `tsc --init` turns it on, which makes array and index-signature reads include `undefined`. If you try an example in a fresh project, that setting can explain an extra error. The repository documents the full configuration.

When a character claims that an approach is better, they show you the code. When another character disagrees, they show you why it breaks. Some examples deliberately produce errors; others reuse definitions from earlier in the conversation or show alternative implementations. The checks account for that context.

You don't need to run the examples to follow along. If you want to, the repository's [README](../../README.md#checking-the-examples) explains how to run the checks with the pinned compiler.
